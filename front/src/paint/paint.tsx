...
const aiMessages = [
  "Coucou ! Je suis Technolog'IA, ton assistant créatif !",
  "Choisis une belle couleur pour commencer ton dessin !",
  "Bravo ! Tu dessines très bien ! Continue comme ça !",
  "N'hésite pas à utiliser différents outils pour créer ton chef-d'œuvre !",
  "Tes couleurs sont magnifiques ! Tu as un vrai talent d'artiste !",
  "Essaie d'ajouter des formes géométriques à ton dessin !",
  "Tu peux aussi écrire du texte sur ton dessin si tu veux !",
  "Fantastique ! Ton dessin prend forme petit à petit !",
  "Continue à explorer ta créativité, tu fais du super travail !",
  "Quand tu auras fini, n'oublie pas de sauvegarder ton œuvre d'art !",
]
...
export default function PaintKids() {
  ...
  const [currentAIMessage, setCurrentAIMessage] = useState(aiMessages[0])
  ...
  const updateAIMessage = (action: string) => {
    ...
    switch (action) {
      case "colorChange":
        newMessage = aiMessages[1]
        break
      case "drawing":
        newMessage = aiMessages[2]
        break
      case "toolChange":
        newMessage = aiMessages[3]
        break
      case "goodWork":
        newMessage = aiMessages[4]
        break
      default:
        newMessage = aiMessages[Math.floor(Math.random() * aiMessages.length)]
    }
        ...
        <div className="relative w-100 bg-white/90 backdrop-blur-sm overflow-hidden p-6 shadow-lg">
          ...
          <div className="absolute bottom-0 flex flex-row justify-around w-full">
            <div className="flex items-center absolute bottom-0 left-0">
              {/* Assistant vocal avec visualiseur */}
              <AIVoiceAssistant
